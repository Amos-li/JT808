...
{
    public class JT808_0x0704_Formatter : IJT808Formatter<JT808_0x0704>
    {
        public JT808_0x0704 Deserialize(ReadOnlySpan<byte> bytes, out int readSize, IJT808Config config)
        {
            int offset = 0;
            ...
            for (int i = 0; i < jT808_0X0704.Count; i++)
            {
                ...
                try
                {
                    JT808_0x0200 jT808_0X0200 = JT808FormatterExtensions.GetFormatter<JT808_0x0200>().Deserialize(bytes.Slice(offset, buflen), out _, config);
        ...
        public int Serialize(ref byte[] bytes, int offset, JT808_0x0704 value, IJT808Config config)
        {
            ...
            foreach (var item in value?.Positions)
            {
                try
                {
                    ...
                    int positionOffset = JT808FormatterExtensions.GetFormatter<JT808_0x0200>().Serialize(ref bytes, offset + 2, item, config);
